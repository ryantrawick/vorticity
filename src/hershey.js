export const FONT_HERSHEY = {
  parseBound: function (entry) {
    let ordR = 'R'.charCodeAt(0)
    let bound = entry.slice(3, 5)
    let xmin = 1 * bound[0].charCodeAt(0) - ordR
    let xmax = 1 * bound[1].charCodeAt(0) - ordR
    return [xmin, xmax]
  },
  putChar: function (c, args) {
    ...
    let entry = args.font[args.cmap(c.charCodeAt(0))]
    if (entry === undefined) {
      return 0
    }
    let cksum = 1 * entry.slice(0, 3)
    let [xmin, xmax] = FONT_HERSHEY.parseBound(entry)
    let content = entry.slice(5)
    //P5.push()
    args.graphics.moveTo(Math.round(-xmin + args.offsetX), args.offsetY)
    //P5.beginShape()
    ...
    for (let i = 0; i < content.length; i += 2) {
      ...
      if (digit === ' R') {
        ...
        //args.graphics.moveTo((x + args.offsetX) * args.scale, (y + args.offsetY) * args.scale)
        next = true
      } else {
        let x = digit[0].charCodeAt(0) - ordR
        let y = digit[1].charCodeAt(0) - ordR
        ...
        // }
        if (next) {
          args.graphics.moveTo(Math.round((x + args.offsetX) * args.scale), Math.round((y + args.offsetY) * args.scale))
          next = false
        } else {
          args.graphics.lineTo(Math.round((x + args.offsetX) * args.scale), Math.round((y + args.offsetY) * args.scale))
        }
        //args.graphics.lineTo((x + args.offsetX) * args.scale, (y + args.offsetY) * args.scale)
        // TODO: Move here?
      }
    }
    //P5.endShape()
  ...
  putText: function (s, args) {
    ...
    if (args.align === 'center') {
      args.offsetX += -(FONT_HERSHEY.estimateTextWidth(s, args) / 2)
    } else if (args.align === 'right') {
      args.offsetX += -FONT_HERSHEY.estimateTextWidth(s, args)
    }
    
    for (let i = 0; i < s.length; i++) {
      let x = FONT_HERSHEY.putChar(s[i], args)
      args.offsetX += x
    }
    //P5.pop()
  },
  estimateTextWidth: function (s, args) {
    ...
    let sum = 0
    for (let i = 0; i < s.length; i++) {
      let entry = args.font[args.cmap(s[i].charCodeAt(0))]
      if (entry === undefined) {
        return 0
      }
      let [xmin, xmax] = FONT_HERSHEY.parseBound(entry)
